...
脚本会安装 Docker、生成独立应用密钥和随机初始管理员密码、构建容器、执行健康检查并启用每日 SQLite 在线备份。首次凭据会在首次构建前直接显示在当前交互终端，请立即存入受控密码管理器；不要通过会永久归档标准输出的无人值守任务运行首次部署。无论部署成功还是失败，脚本都会清空 `.env` 中的初始密码；失败时还会移除可能携带该环境变量的 app 容器。若首次运行在数据库创建前失败，重新运行会生成并显示一组新密码。服务器不会保存管理员明文密码。
